...
const poedbBorderSource = "https://poedb.tw/us/Maiden_Voyage#DeepWaterBorderMods";
assert.equal(core.catalog.catalog.status, "live-poedb-verified");
assert.equal(core.catalog.catalog.verifiedAt, "2026-08-07");
assert.ok(core.catalog.catalog.sources.includes(poedbBorderSource));
assert.equal(core.catalog.borderModifiers.length, 65, "Runtime catalog should match PoEDB's advertised 65 border-mod rows");
assert.equal(new Set(core.catalog.borderModifiers.map((modifier) => modifier.id)).size, 65, "Every PoEDB border-mod row needs a stable unique ID");
assert.equal(core.catalog.borderModifiers.filter((modifier) => modifier.sourceText).length, 64, "PoEDB currently exposes text for 64 of its 65 border-mod rows");
assert.equal(core.catalog.borderModifiers.find((modifier) => modifier.id === "magic-monster-mods2").sourceStatus, "poedb-blank-row");
assert.equal(core.borderModifierOptions().find((modifier) => modifier.id === "pack-size1").summary, "16% increased Pack Size in adjacent Areas");
...
const touchingPackBorderMods = Array.from({ length: 9 }, () => ({}));
touchingPackBorderMods[0] = { north: "pack-size1" };
const touchingPackBorderEvaluation = core.evaluateBoard(touchingBorderBoard, "general", [], touchingPackBorderMods);
assert.equal(touchingPackBorderEvaluation.appliedModifiers[0].some((modifier) => modifier.label === "Increased Pack size"), false, "PoEDB adjacent Pack Size border mods should not apply to their source tile");
assert.equal(touchingPackBorderEvaluation.appliedModifiers[1].some((modifier) => modifier.label === "Increased Pack size" && modifier.scope === "connected-border"), true, "PoEDB adjacent Pack Size border mods should cross a reciprocal touching line");
